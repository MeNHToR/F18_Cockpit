...
void loop() {
  ...
  if (comCommRelaySw_Cipher == HIGH || !comCommRelaySw == 0) {
    sendDcsBiosMessage("COM_COMM_RELAY_SW","0");
    comCommRelaySw = 0;
  }
  else if (comCommRelaySw_Plain==HIGH || !comCommRelaySw == 2) { 
    sendDcsBiosMessage("COM_COMM_RELAY_SW","2");
    comCommRelaySw = 2;
  }
  else if (comCommRelaySw_Cipher==LOW || comCommRelaySw_Plain==LOW || !comCommRelaySw == 1){ 
